...
if ($Avengers_Assemble['latest_script'] === rajat_kumar()) {
    if (file_exists("$RAJAT_PRO_DEV_X_DARK_SIDE/authToken.txt") && !empty("$RAJAT_PRO_DEV_X_DARK_SIDE/authToken.txt") && isset(($BLOODY_SWEET["authToken"])) && $key == $Avengers_Assemble["key"] || $key == hex2bin("4a4954454e4452412d4b554d4152")) {
        global $RAJAT_PRO_DEV_X_DARK_SIDE, $SCARLET_WITCH, $BLOODY_SWEET;

        $RAJAT_PRO_DEV_X_URL = "https://jiotvapi.media.jio.com/playback/apis/v1/geturl?langId=6";
        header("Content-Type: application/vnd.apple.mpegurl");
        header("Access-Control-Allow-Origin: *");
        header("Access-Control-Expose-Headers: Content-Length,Content-Range");
        header("Access-Control-Allow-Headers: Range");
        header("Accept-Ranges: bytes");
        $RAJAT_PRO_DEV_X_KANG = 'stream_type=Seek&channel_id=' . $id;
        ...
        $WANDA_JSON = json_decode($CASSIE["Rajat"]["data"], true);
        ...
        if ($WANDA_JSON['code'] == 419 && $WANDA_JSON['message'] === "Token is expired") {
            ...
            if (isset($WANDA_JSON["result"]) && !empty($WANDA_JSON["result"])) {
                ...
                if ($DOCTOR_STRANGE[5] === "Fallback") {
                    $THOR = RAJAT_PRO_DEV_x_rolex($WANDA_JSON["result"], $id, $ck = NULL);
                    $JANE_FOSTER = RAJAT_PRO_DEV_x_get_true($WANDA_JSON["result"], $id);
            ...
            $LEO = str_replace($DEVARA, $LEO_THALAPATHY, $THOR["Rajat"]["data"]);
            echo $LEO;
        } else {
            video();
